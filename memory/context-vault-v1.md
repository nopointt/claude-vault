---
# context-vault-v1.md — V-01 Rename + Ship v0.2.0
...
| Source code | `nospace/development/context-vault/src/` + `bin/` | 9 files, ~500 LoC |
| Old repo URL | `github.com/nopointt/claude-vault` | Pending rename |
| New repo URL | `github.com/nopointt/context-vault` | After rename |

## Active Decisions (locked)

- **D-V01-01:** Single-direction rename, no backward compat. Users must `mv ~/.claude-vault ~/.context-vault` manually. Migration section in README documents the steps.
...
- **D-V01-04:** Log prefix unified to `[context-vault]`. Previously split `[claude-vault]` for CLI startup + `[vault-proxy]` for request handlers — inconsistent and confusing.
...
- **D-V01-06:** Publish as `context-vault` (unscoped) on npm. Fallback to `@nopoint/context-vault` if name taken.
- **D-V01-07:** Launch channels deferred to V-05 (GitHub README + HN Show HN + r/ClaudeAI + r/selfhosted).
- **D-V01-08:** Directory at `nospace/development/context-vault/` (not `tools/`). Has memory/, roadmap, epics — constitution-level visibility.
...
- **D-V01-10:** Bug audit preserves findings B-01..B-17 in `context-vault-backlog.md`. Each bug has explicit V-0X epic assignment. No silent fixes.
...
**Action:** Copy `nospace/tools/claude-vault/` → `nospace/development/context-vault/`.
...
- [x] Verification: `ls development/context-vault/package.json` → OK
- [x] Verification: `git log --oneline -3` in new dir shows history

### P2 — Code Rename ✅ COMPLETE

**Action:** Replace all `claude-vault` references in code with `context-vault`. Rename binary. Update env var. Update log prefixes.

**Files touched:**
- `package.json`: name, version 0.1.0→0.2.0, description, bin, files[], keywords[], bugs, homepage, repository
- `bin/claude-vault.ts` → `bin/context-vault.ts` (git mv) with internal refs updated
...
- `.npmignore`: added `.context-vault/`, `memory/`
...
- [x] README starts with `# context-vault`
- [x] package.json version 0.2.0
- [x] Binary name `context-vault` in package.json bin
...
- [x] Hook paths point to `development/context-vault/src/hooks/`
- [x] `ANTHROPIC_BASE_URL=http://127.0.0.1:9277` in settings.json env
- [x] `.claudeignore` has `.context-vault/` (`.claude-vault/` kept for transition)
...
- [x] `memory/context-vault-about.md` (L1, 166 lines)
- [x] `memory/context-vault-roadmap.md` (L2, 158 lines)
- [x] `memory/context-vault-v1.md` (L3 — this file, 288 lines)
- [x] `memory/context-vault-backlog.md` (L2.5 — 18 bugs + 18 TD, 233 lines)
...
- [x] `memory/chronicle/context-vault-current.md` (4080 bytes after CLOSE drain)
...
**Action:** Catalogue all bugs found during source review. 17 findings from recovery §5. Formalize in `context-vault-backlog.md`.
...
**Done when:**
- [ ] All 17 bugs in `context-vault-backlog.md` with severity + V-0X assignment
...
**Done when:**
- [ ] All items in `context-vault-backlog.md` TD section
- [ ] Each item has V-0X assignment

### P7 — V-02..V-06 Epic Formalization ✅ LISTED

**Action:** List V-02..V-07 epics in roadmap (`context-vault-roadmap.md`).
...
- [ ] Check `npm view context-vault` — is name available? If not, switch to `@nopoint/context-vault`
- [ ] `npm adduser` if first time publishing from this machine (manual by nopoint)

**Verify:**
```bash
cd /c/Users/noadmin/nospace/development/context-vault
bun publish --dry-run 2>&1 | head -30
# Expected: list of files ~= [bin/*, src/*, README.md, LICENSE, package.json]
npm view context-vault 2>&1 | head -5
...
**Action:** Commit V-01 in logical atomic units in context-vault repo (at new `development/context-vault/` location).

**Commits (in order):**
1. `feat(rename): claude-vault → context-vault (v0.2.0)` — package.json, bin rename, src code changes, .npmignore
   - GSD-Task: `V-01 / P2`
...
3. `docs: rewrite README for context-vault v0.2.0 with install + migration + troubleshooting` — README.md
   - GSD-Task: `V-01 / P2`
...
| AC-3 | Binary works: `context-vault init/start/stop/add/remove/list/status` all functional | Manual CLI smoke test |
...
| AC-7 | All 17 bugs catalogued in backlog with severity + epic | `grep -c "^- \*\*B-" context-vault-backlog.md` ≥ 17 |
| AC-8 | Tech debt items catalogued | `grep -c "^- \*\*TD-" context-vault-backlog.md` ≥ 10 |
...
|---|---|---|
| npm name `context-vault` taken | MEDIUM | Fallback to `@nopoint/context-vault` scoped |
...
| User doesn't migrate `~/.claude-vault/` to `~/.context-vault/` | LOW | README migration section documents. `context-vault init` creates new dir automatically. Old vault data non-critical — can be re-added. |
...
| File | Owner |
|---|---|
| `memory/context-vault-v1.md` (this L3) | Axis — epic progress |
| `memory/context-vault-backlog.md` | Axis — bug/TD audit |
...
1. Finish P4 — write `context-vault-backlog.md` + chronicle files + session-scratch placeholder
...
7. nopoint manual: GitHub repo rename, `mv ~/.claude-vault ~/.context-vault`, `npm publish` if ready
